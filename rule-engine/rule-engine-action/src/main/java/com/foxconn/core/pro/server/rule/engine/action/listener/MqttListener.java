...
public class MqttListener implements BaseListener
{
	...
	public void action(JSONObject parameter, JSONObject bean,JSONObject systemData)
	{
		try
		{
			...
			Integer qos = parameter.getInteger("qos");

			if (topic == null)
			{
				log.error("mqtt Listener topic is null:" + (parameter != null ? parameter.toJSONString() : "") + ","
						+ (bean != null ? bean.toJSONString() : ""));
				return;
			}
			if (qos == null)
			{
		...
		{
			log.error("Mqtt Listener Exception:" + (parameter != null ? parameter.toJSONString() : "") + ","
